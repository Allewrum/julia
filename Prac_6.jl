...
function oneside(P::Vector2D{T}, Q::Vector2D{T}, s::Segment2D{T})::Bool where T
	# l - направляющий вектор является прямым.
	l = s.B - s.A
...
function intersection(s1::Segment2D{T},s2::Segment2D{T})::Union{Vector2D{T},Nothing} where T
	...
	#Если матрица A - вырожденная, то произойдет ошибка
...
function inside(point::Vector2D{T},polygon::AbstractArray{Vector2D{T}})::Bool where T
...
println("Внутри: ",inside( (x=0,y=0),[(x=0,y=1),(x=1,y=-1),(x=-1,y=-1)] ))
println("Внутри: ",inside( (x=5,y=0),[(x=0,y=1),(x=1,y=-1),(x=-1,y=-1)] ))
