...
# № 3.1 Первый вариант - генерация двоичных кодов чисел 0, 1, ..., 2^n-1
...
# № 3.2 Второй варинт - на основе непосредственной генерации последовательности индикаторов в лексикографическом порядке
...
# next_rep_plasement(c::Vector, n) - Генерация размещений с повторениями
# next_permute(p::AbstractVector) - Генерация перестановок
# next_indicator(indicator::AbstractVector{Bool}) - Генерация всех подмножеств
# next_indicator(indicator::AbstractVector{Bool}, k) - Генерация k-элементных подмножеств
# next_split(s::AbstractVector{Integer}, k) - Генерация разбиений
 
# Абстрактный тип для генерации комбинаторных объектов
